...
class MixVisionTransformer(nn.Module):
    def __init__(self, in_chans=4, num_classes=1000, embed_dims=[32, 64, 160, 256],
        ...
        #----------------------------------------------------------------------------------------------------------------------------------------
        #   block1  7x7  四倍下采样  512, 512, 3 => 128, 128, 32 => 16384, 32
        #----------------------------------------------------------------------------------------------------------------------------------------
        #-----------------------------------------------
        #   对输入图像进行分区，并下采样
        ...
        #----------------------------------------------------------------------------------------------------------------------------------------
        #   block2  接block1  3x3  两倍下采样  128, 128, 32 => 64, 64, 64 => 4096, 64
        #----------------------------------------------------------------------------------------------------------------------------------------
        #-----------------------------------------------
        #   对输入图像进行分区，并下采样
        ...
        #----------------------------------------------------------------------------------------------------------------------------------------
        #   block3  接block2  3x3  两倍下采样  64, 64, 64 => 32, 32, 160 => 1024, 160
        #----------------------------------------------------------------------------------------------------------------------------------------
        #-----------------------------------------------#
        #   对输入图像进行分区，并下采样
        ...
        #----------------------------------------------------------------------------------------------------------------------------------------
        #   block4  接block3  3x3  两倍下采样  32, 32, 160 => 16, 16, 256 => 256, 256
        #----------------------------------------------------------------------------------------------------------------------------------------
        #-----------------------------------------------#
        #   对输入图像进行分区，并下采样
        ...
        for name, param in state_dict.items():
            if name in own_state:
                if own_state[name].shape == param.shape:
                    ...
                    print("Loaded parameter:", name)
                else:
                    print("Skipped parameter (shape mismatch):", name)
            else:
                print("Skipped parameter (not found in own_state):", name)
        if strict:
            missing = set(own_state.keys()) - set(state_dict.keys())
            if len(missing) > 0:
                raise KeyError("Missing keys in own_state: " + ', '.join(missing))
    def forward(self, x):
        B = x.shape[0]
        outs = []
        ...
        #----------------------------------#
        #   block1
        #   512, 512, 3 => 256, 256, 3 => 128, 128, 32 => 4096, 32
        #----------------------------------#
        x, H, W = self.patch_embed1.forward(x)
        for i, blk in enumerate(self.block1):
            x = blk.forward(x, H, W)
        x = self.norm1(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()  # 将展开的特征进行转置  B, C, H, W
        outs.append(x)
...
class mit_b0(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            ...
            self.load_state_dict(torch.load("/officialWeights/segformer/segformer_b0_backbone_weights.pth"), strict=False)
...
class mit_b1(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(torch.load("/officialWeights/segformer/segformer_b1_backbone_weights.pth"), strict=False)
...
class mit_b2(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(torch.load("/officialWeights/segformer/segformer_b2_backbone_weights.pth"), strict=False)
...
class mit_b3(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(torch.load("/officialWeights/segformer/segformer_b3_backbone_weights.pth"), strict=False)
...
class mit_b4(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(torch.load("/officialWeights/segformer/segformer_b4_backbone_weights.pth"), strict=False)
...
class mit_b5(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(torch.load("/officialWeights/segformer/segformer_b5_backbone_weights.pth"), strict=False)